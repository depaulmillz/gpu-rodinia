...
#if REORDER_TREE
  addr =
      (arr.x | ((arr.z & 0xF) << 8)) | ((arr.y | ((arr.z & 0xF0) << 4)) << 16);
#else
  addr = MKI(arr);
#endif
}
...
__global__ void mummergpuRCKernel(MatchCoord *match_coords, char *queries,
  ...
  min_match_len + 1); queries += qryAddr;
  ...
  #if !REFTEX
                                                          //FIXME: this needs to
  be a pointer to ref->d_ref_array ,NULL #endif
  );
  ...
  r, qry_match_len, c);
                          ...
                          if (r != c)
                          {
                             // mismatch on edge
                             XPRINTF("mismatch on edge: %d, edge_pos: %d\n",
  qry_match_len,refpos - (MKI(node.start))); goto RECORD_RESULT;
                          }
