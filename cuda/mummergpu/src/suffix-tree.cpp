  ...
  void buildUkkonen() {
    ...
    for (int i = 2; i <= len; i++) {
      ...
      for (int j = startj; j <= i; j++) {
        ...
        //       suffix link. 3) Follow suffix link which shifts the path from
        //       S[j-1..i] to S[j..i] 4) Walk down tree in new location ensuring
        //       S[i-w .. i] is in tree
              ...
              if (m_string[betapos] == nodebase) {
                ...
                if (betapos == i + 1) {
                  ...
                  if ((nodewalk == child->len(i)) && (child->m_end == len)) {
                    ...
                    // round Start the next round at the last explicit extension
                    if (DOPHASETRICK) {
                      startj = j;
                    ...
                    if (DOINTERNALSKIP) {
                      ...
                      // every other extension in this phase will also hit an
                      // internal match.

                      // Have to be careful since leafs get the full string
                      // immediately, but they really have a Rule 1 extension
      ...
      if (b == '>') {
        ...
        if (offsetspos != 0) {
          if (this_qrylen < min_match_length) {
            ...
            // padding For now, we are overestimating.
            curr_mem_usage +=
