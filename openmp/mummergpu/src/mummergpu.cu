...
void loadReferenceTexture(MatchContext *ctx) {
  ...
  if (!ctx->on_cpu) {
    ...
    CUDA_SAFE_CALL(cudaMemcpy((void *)(ref->d_ref_array), ref->str, ref->len,
                              cudaMemcpyHostToDevice));
    ...
    CUDA_SAFE_CALL(cudaMemcpy2D((ref->d_ref_array), refpitch, ref->h_ref_array,
                                ref->pitch, ref->pitch * sizeof(char), numrows,
                                cudaMemcpyHostToDevice));
...
void loadReference(MatchContext *ctx) {
  ...
  if (!ctx->on_cpu) {
    ...
    CUDA_MALLOC_ARRAY((cudaArray **)(&ref->d_node_tex_array), &nodeTextureDesc,
                      ref->tex_width, ref->tex_node_height);
    ...
    CUDA_MALLOC_PITCH((void **)(&ref->d_node_tex_array), &nodepitch,
                      ...
                      ref->tex_node_height);
    CUDA_SAFE_CALL(cudaMemcpy2D((ref->d_node_tex_array), nodepitch,
                                ref->h_node_tex_array, nodepitch,
                                ref->tex_width * sizeof(PixelOfNode),
    ...
    if (ref->tex_children_height) {
      ...
      CUDA_SAFE_CALL(cudaMemcpyToArray(
          (cudaArray *)(ref->d_children_tex_array), 0, 0,
      ...
      CUDA_MALLOC_PITCH((void **)(&ref->d_children_tex_array), &childpitch,
                        ref->tex_width * sizeof(PixelOfChildren),
                        ref->tex_children_height);
      CUDA_SAFE_CALL(cudaMemcpy2D(
          (ref->d_children_tex_array), childpitch, ref->h_children_tex_array,
          childpitch, ref->tex_width * sizeof(PixelOfNode),
          ref->tex_children_height, cudaMemcpyHostToDevice));
#endif
    }
    ...
    if (ref->tex_children_height) {
      ...
      CUDA_SAFE_CALL(
          cudaMemcpy((ref->d_children_tex_array), ref->h_children_tex_array,
                     ref->tex_children_height * sizeof(PixelOfChildren),
                     cudaMemcpyHostToDevice));
#if CHILDTEX
    ...
    CUDA_SAFE_CALL(
        cudaMemset((ref->d_node_hist), 0, ref->tex_node_height * sizeof(int)));
...
void loadQueries(MatchContext *ctx) {
  ...
  if (!ctx->on_cpu) {
    ...
    CUDA_SAFE_CALL(
        cudaMemcpy((void *)queries->d_lengths_array, queries->h_lengths_array,
                   numQueries * sizeof(int), cudaMemcpyHostToDevice));
    stopTimer(toboardtimer);
...
void loadResultBuffer(MatchContext *ctx) {
  ...
  if (!ctx->on_cpu) {
    ...
    CUDA_SAFE_CALL(cudaMemcpy(
        (void *)ctx->results.d_coord_tex_array, ctx->results.h_coord_tex_array,
        numQueries * sizeof(int), cudaMemcpyHostToDevice));
#endif
    stopTimer(toboardtimer);
...
void transferResultsFromDevice(MatchContext *ctx) {
  if (!ctx->on_cpu) {
    ...
    CUDA_SAFE_CALL(cudaMemcpy(
        ctx->results.h_match_coords, ctx->results.d_match_coords,
    ...
    CUDA_SAFE_CALL(cudaMemcpy(ctx->ref->h_node_hist, ctx->ref->d_node_hist,
                              ctx->ref->tex_node_height * ctx->ref->tex_width *
                                  sizeof(int),
    ...
    if (ctx->statistics.node_hist_size <
      ...
      int *temp = (int *)calloc(ctx->ref->tex_width * ctx->ref->tex_node_height,
                                sizeof(int));
      if (ctx->statistics.node_hist_size)
    ...
    if (ctx->statistics.child_hist_size <
      ...
      temp = (int *)calloc(ctx->ref->tex_width * ctx->ref->tex_children_height,
                           sizeof(int));
...
void runPrintKernel(MatchContext *ctx, ReferencePage *page,
  ...
  if (DEBUG) {
    for (int i = 0; i < numMatches; i++) {
      printf("m[%d]:\t%d\t%d\t%d\t%d\t%d\t%d\n", i, h_matches[i].resultsoffset,
             h_matches[i].queryid, h_matches[i].matchnode.data,
             h_matches[i].numLeaves, h_matches[i].edgematch,
...
void getExactAlignments(MatchContext *ctx, ReferencePage *page, bool on_cpu) {
  ...
  while (next_coord < last_coord) {
    ...
    coordsToPrintBuffers(ctx, page, &h_matches, &h_alignments, boardFreeMemory,
                         &next_coord, &numMatches, &numAlignments, &nextqry,
                         &nextqrychar);
    stopTimer(btimer);
    ...
    fprintf(stderr,
            "Round %d: Printing results for match coords [%d-%d) of %d using "
            "%d matches and %d alignments\n",
    ...
    for (int m = 0; m < numMatches; m++) {
      ...
      for (int i = 0; i < h_matches[m].numLeaves; i++) {
        ...
        sprintf(buf, "%d\t%d\t%d\n", h_alignments[base + i].left_in_ref,
                h_matches[m].qrystartpos + 1, h_alignments[base + i].matchlen);
        addToBuffer(buf);
  ...
  fprintf(stderr,
          "Finished processing %d matches and %d potential alignments in %d "
          "rounds\n",
...
void matchOnCPU(MatchContext *ctx, bool doRC) {
  ...
  if (doRC) {
    ...
    computeGold(&ctx->results, ctx->ref->str, ctx->queries->h_tex_array,
                ctx->queries->h_addrs_tex_array, ctx->queries->h_lengths_array,
